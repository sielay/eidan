...
def assemble_plugin_frontends(context: Path) -> Path:
    """Copy each plugin's declared frontend package into apps/web and
    generate the registry the host consumes (eidan#284, frontend half).

    For every ``plugins/<name>/plugin.yaml`` with a ``frontend.package``,
    copy that package's contents to
    ``apps/web/src/plugins/<name>/`` and collect its ``routes`` (mounted
    under ``/p/<name>/...`` by the host catch-all) + ``components`` (named
    slots). Writes ``apps/web/src/plugins/registry.generated.ts`` — the
    single module the host imports. Because assembly happens at
    build-context time (before ``next build``), Next.js compiles the
    plugin code statically; there is no runtime route registration.

    Returns the generated registry path. Idempotent: the target dir is
    rebuilt from scratch each call.
    """
    plugins_src = context / "plugins"
    web_plugins = context / "apps" / "web" / "src" / "plugins"
    if web_plugins.exists():
        shutil.rmtree(web_plugins)
    web_plugins.mkdir(parents=True)

    routes: list[tuple[str, str, str]] = []  # (plugin, path, import_spec)
    slots: list[tuple[str, str, str]] = []  # (plugin, slot, import_spec)
...
def _render_frontend_registry(
    ...
    """Render the generated TS registry of plugin routes + slot components.

    A TS string-emitter (not codegen from a schema): the import specifiers
    must be literals so the Next.js bundler can statically resolve them, so
    this is intentionally hand-rendered rather than JSON.
    """
    lines = [
        "// SPDX-License-Identifier: AGPL-3.0-or-later",
    ...
    for plugin, path, spec in routes:
        lines.append(
            f"  {{ plugin: {plugin!r}, path: {path!r}, "
            f"load: () => import({spec!r}) }},"
        )
    lines.append("];")
    lines.append("")
    lines.append("export const pluginSlots: PluginSlotEntry[] = [")
    for plugin, slot, spec in slots:
        lines.append(
            f"  {{ plugin: {plugin!r}, slot: {slot!r}, "
            f"load: () => import({spec!r}) }},"
        )
    lines.append("];")
    lines.append("")
