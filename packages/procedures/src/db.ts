...
// Principal-stamping transaction helper (same shape as @eidandev/memory's Db — three similar
// helpers beat one leaky abstraction). Every read/write runs with eidan.current_user_id set from
// the ambient matbot Principal, so RLS isolates tenants where it is enforced.
export class Db {
