...
// Principal-stamping transaction helper (mirrors @eidandev/routines / @eidandev/memory): per-user
// reads/writes run with eidan.current_user_id set from the ambient Principal. Agents also scope every
// statement by an explicit user_id predicate, so isolation holds with or without RLS policies. The
// cross-user dispatch loop uses the plain `query` path (no principal) to scan every owner's agents.
export class Db {
